...
survival time of indiviudals. If the lines cross then it is most probable that the two are not proportional, thereby 
violating the proportional hazards assumption.

**Note:** The Kaplan-Meier curve method of testing the proportional hazards assumption is not well suited to small data 
datasets where, consequently, curves may overlap without the proportional haards assumption having been violated.
...
<!-- Plot "logarithm of negative logirhtm of estimated survivor function" (see notes) -->

R provides a way for us to calculate the proportional hazard assumption using `cox.zph`.

In *linear regression* we have the Residual Sum of Squares (RSS) which we can use to see how well data fits a model that 
we have built. Analogous to this is the `-2log likelihood` 
